...
Legal OS v0.7.0 release candidate contains fourteen installable Skills. T-routes describe capabilities; not every route is a separate Skill directory. `legalos.manifest.json` is the machine authority for this inventory.
...
| T-05 | Current law and case research | `cn-law-hub` for current law; `cn-case-hub` for cases | Official/authoritative source verification required; access controls may not be bypassed. |
...
## v0.7.0 compatibility note

T-01 through T-12 remain stable. `cn-case-hub` retains the v0.6 case-record fields while adding a dual-axis case matrix and decision-fork variables. T-05 is no longer dependent on an unbundled current-law Skill: current-law questions dispatch to bundled `cn-law-hub`. The public package still requires human legal judgment and separate authorization for external actions.
