...
Legal OS v0.8.0 public adaptation contains fourteen installable Skills. T-routes describe capabilities; not every route is a separate Skill directory. `legalos.manifest.json` is the machine authority for this inventory.
...
| T-05 | Current law and case research | `cn-law-hub` for current law and official-source query/record validation; `cn-case-hub` for cases | Official/authoritative source verification required; access controls may not be bypassed; public package does not bundle crawlers or external database SDKs. |
...
T-01 through T-12 remain stable. `cn-case-hub` retains the v0.6 case-record fields while adding a dual-axis case matrix and decision-fork variables. T-05 current-law questions dispatch to bundled `cn-law-hub`, whose public adaptation generates official-source plans and validates authority records without embedding external crawlers. The public package still requires human legal judgment and separate authorization for external actions.
