...
状态：**v0.8.0 公开候选，尚未发布**。本节记录当前候选相对于已发布公开基线的适配变化。
...
- 13 个公开法律 DOCX 模板更新为当前候选来源包，并保留原始 DOCX 的 OOXML、文本、表格、编号、样式和其他包级特征；模板目录明确标记为 `candidate`，不表示已启用或已发布。

### 当前候选验证

- 公开表面、仓库结构、路由、模板包/哈希、公开边界、标准库运行时和全仓测试均在本地候选门中验证；详细命令与未执行的外部发布/在线访问项见 [`TEST_REPORT_v0.8.0.md`](TEST_REPORT_v0.8.0.md)。
...
状态：**历史已发布公开适配基线（公开预发布，非稳定版）**；当前工作树的 v0.8.0 候选尚未重新发布。

### 现行法公开适配

- `cn-law-hub` 增加机器可读的十个主要官方来源登记，并由查询计划脚本逐源输出官方入口、权限范围和检索词。
- authority record 校验增加必备元数据字段、官方 HTTPS 主机校验、法源类型绑定、日期字段和重复链接检查。
- 历史基线的 T-05 路由为 `current-law-research → cn-law-hub`、`case-research → cn-case-hub`；当前候选的路由变化见上方 `[Unreleased]`。
