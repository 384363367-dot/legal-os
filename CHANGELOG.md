...
状态：**已发布公开预发布版，非稳定版**。本次是对 v0.8.1 成品表达规则的窄幅收紧，不增加 Skill、路由或模板。

- 完成的 Word 文件及其正文、标题、副标题、页眉页脚、文件名、版本标签、附件名、目录和封面，均不得出现“草案”“送审稿”“修订稿”等历史版本信息，也不得出现内部沟通/意见、修改或删除过程、生成过程等标签。
- 保留影响结论成立的实体事实和法律上必要的程序日期；规则继续由共享的 external-expression canonical reference 统一承载。
- 扩展现有静态回归测试，确认历史版本标签和内部过程标签在全部成品位置均被拦截，并确认不再保留旧的正式用途例外。

验证结果见 [`TEST_REPORT_v0.8.2.md`](TEST_REPORT_v0.8.2.md)；版本范围见 [`RELEASE_NOTES_v0.8.2.md`](RELEASE_NOTES_v0.8.2.md)。
发布 Tag：`v0.8.2`。

## [v0.8.1] - 2026-09-23

状态：**已发布公开预发布版，非稳定版**。本次只收紧现有规则并补充回归测试；Skill、路由和模板数量不变。

- 跨工作流质量门增加长文语义去重、事项风险重要性排序、证据证明层级及正式成品表达检查；任务级 R0–R3 定义保持不变。
- 合同工作流增加明确不能修改时的审核意见路径，履约建议与审批结论分别输出；原有修订路径仍按用户指令触发。
- 领导风险方案报告与中文信息图交付规则更明确，重点前置、阶段按需使用、中文文字保持可校验。
- 回归测试覆盖重复风险唯一归位、简短承接、合同 no-redline、标题及文件包装、中文图片文字和证据升级边界。

验证结果见 [`TEST_REPORT_v0.8.1.md`](TEST_REPORT_v0.8.1.md)；版本范围见 [`RELEASE_NOTES_v0.8.1.md`](RELEASE_NOTES_v0.8.1.md)。
发布 Tag：`v0.8.1`。
