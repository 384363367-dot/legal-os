...
> **2026-07-21 · v0.2.0 候选版（`main`）**

- 诉状、申请书和答辩状现在与独立证据目录成对解析和生成；
- 新增劳动人事争议仲裁申请书、答辩状专用模板，避免与商事仲裁模板混用；
- 公开模板总数更新为 24 个，并继续执行哈希绑定和版式审计；
- 当前 `main` 已包含上述更新，最新公开提交为 [`00dff1a`](https://github.com/384363367-dot/legal-os/commit/00dff1a12dfeb5a307a839ef80e28a5e72e771fc)。

**下载提示：** 从默认 `main` 分支克隆或点击 **Code → Download ZIP**，获得当前 v0.2.0 候选内容；从 [GitHub Releases](https://github.com/384363367-dot/legal-os/releases) 下载，获得最近一次稳定公开版 v0.1.1。候选版可能继续调整，生产使用前请执行仓库验证和事项级法律审查。
...
- 10 个公开 Legal OS Skills 及其工作流参考；
- 24 个去身份化 Office 模板、模板注册表、成对模板解析器和版式审计脚本；
- `legalos.manifest.json` 机器可读 Kernel 清单及 Schema；
- 工作空间和任务分流文档；
- `route-only` / `route-and-run` 统一入口契约及 15 个合成场景测试；
- 合同修订质量闸门与指标脚本；
- 文档、脚本与合成安全测试指引。

证据映射和现行法核验在 v0.2.0 中属于工作流阶段或外部依赖，并非两个遗漏的独立 Skill。机器权威见 [`legalos.manifest.json`](legalos.manifest.json)，完整说明见 [`docs/capability-matrix.md`](docs/capability-matrix.md)。
...
将 `skills/` 下需要的完整目录复制到 `$CODEX_HOME/skills/`，然后重新启动 Codex。建议先安装 `legal-os-unified-intake` 和 `legal-os-template-runtime`，再按 [`docs/capability-matrix.md`](docs/capability-matrix.md) 选择业务 Skill。模板选择与审计规则见 [`docs/template-runtime.md`](docs/template-runtime.md)。
...
当前开发线为预发布候选版本（v0.2.0），尚未发布；当前稳定公开版本仍为 v0.1.1。在首个稳定版本发布前，接口、模块边界和仓库结构仍可能调整。版本变更见 [`CHANGELOG.md`](CHANGELOG.md)。
