...
> **2026-07-23 · v0.3.0 公开预发布版**

- 新增跨工作流 `legal-quality-gate`，在正式法律成果交付前统一检查事实、证据、现行法、表达边界、隐私、人工复核和文件完整性；
- 新增确定性发布锁，以 `BLOCKED`、`DRAFT_ONLY`、`RELEASE` 三种状态区分阻断项、可审阅草稿和可交付成果；
- 提供 11 个可安装 Skill、24 个去身份化 Office 模板、15 个合成路由场景和 39 项自动化测试；
- 强化公共包边界：公开内容仅包含通用工作流、合成测试材料和可再分发资源。

**下载：** 请从 [GitHub Releases](https://github.com/384363367-dot/legal-os/releases/tag/v0.3.0) 获取源码或一键安装包 `LegalOS-Skills-v0.3.0.zip`。
...
- 11 个可安装的 Legal OS Skills 及其工作流参考；
...
证据映射和现行法核验在 v0.3.0 中分别作为工作流阶段和外部权威检索依赖提供，而不是两个独立 Skill。机器可读清单见 [`legalos.manifest.json`](legalos.manifest.json)，完整说明见 [`docs/capability-matrix.md`](docs/capability-matrix.md)。
...
当前公开预发布版本为 v0.3.0。在首个稳定版本发布前，接口、模块边界和仓库结构仍可能调整。版本变更见 [`CHANGELOG.md`](CHANGELOG.md)。
