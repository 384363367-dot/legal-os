...
证据映射和现行法核验在 v0.1.1 中属于工作流阶段或外部依赖，并非两个遗漏的独立 Skill。完整对应关系见 [`docs/capability-matrix.md`](docs/capability-matrix.md)。
...
当前为预发布版本（v0.1.1）。在首个稳定版本发布前，接口、模块边界和仓库结构仍可能调整。版本变更见 [`CHANGELOG.md`](CHANGELOG.md)。
