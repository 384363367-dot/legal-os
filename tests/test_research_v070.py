...
class ResearchV070Tests(unittest.TestCase):
 ...
 def test_authority_record_validator_accepts_official_record(self):
  rec={'title':'示例规范','authority_type':'law','issuing_body':'全国人民代表大会','status':'effective','proposition':'示例命题','source_url':'https://flk.npc.gov.cn/','accessed_at':'2026-08-23','verification_status':'verified-source','limitations':[],'supersedes':[],'superseded_by':[]}
 ...
 def test_non_official_verified_authority_rejected(self):
  rec={'title':'x','authority_type':'law','issuing_body':'x','status':'effective','proposition':'x','source_url':'https://example.com/x','accessed_at':'2026-08-23','verification_status':'verified-source','limitations':[]}
  with tempfile.TemporaryDirectory() as td:
 ...
 def test_treaty_record_accepts_official_mfa_source(self):
  rec={'title':'示例条约','authority_type':'treaty','issuing_body':'中华人民共和国外交部','status':'effective','proposition':'示例条约命题','source_url':'https://treaty.mfa.gov.cn/','accessed_at':'2026-08-24','verification_status':'verified-source','limitations':[],'supersedes':[],'superseded_by':[]}
  with tempfile.TemporaryDirectory() as td:
   p=Path(td)/'a.json';p.write_text(json.dumps({'authorities':[rec]},ensure_ascii=False),encoding='utf-8');self.run_script(LAW/'scripts/validate_authority_records.py',str(p))
 def test_invalid_authority_type_rejected(self):
  rec={'title':'x','authority_type':'case','issuing_body':'x','status':'effective','proposition':'x','source_url':'https://www.gov.cn/','accessed_at':'2026-08-24','verification_status':'verified-source','limitations':[]}
  with tempfile.TemporaryDirectory() as td:
   p=Path(td)/'a.json';p.write_text(json.dumps({'authorities':[rec]},ensure_ascii=False),encoding='utf-8');self.run_script(LAW/'scripts/validate_authority_records.py',str(p),expected=1)
