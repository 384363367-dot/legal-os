...
When the wording is intended for a counterparty or opponent, apply the public [external-expression boundary](../legal-os-unified-intake/references/external-expression-boundary.md). Keep internal, client and management analysis complete; the boundary selects necessary external disclosure by audience and does not replace source verification or authorization.
...
6. For external wording, check that the message does not volunteer an unraised defence, nonessential legal route, evidence reserve, negotiation floor, or fallback option; retain a necessary response when an actual issue, legal/procedural duty, or truthful non-contradiction requires it.
7. Match the channel: short and copyable for WeChat; subject/paragraph/action structure for email; opening goal, known facts, safe confirmations, and hold points for calls; issue/impact/options/recommendation/owner for internal notes.
8. Preserve a version and evidence/attachment trail when the communication matters.
9. When the requested deliverable is a standard DOCX communication artifact rather than copyable plain text, use `legal-os-template-runtime` to resolve and hash-check the channel template. Preserve its fixed shell while expanding the body for the actual audience and issue; stop with `TEMPLATE_REQUIRED` instead of creating an unrelated layout. Check the resulting DOCX source structure, content and format properties.
