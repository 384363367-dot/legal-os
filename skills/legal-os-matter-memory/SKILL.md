...
9. When producing a standard DOCX matter card, dynamic-fact record or candidate-diff artifact, use `legal-os-template-runtime` to resolve and hash-check the registered template. Preserve the fixed shell, add rows or sections as needed, and keep case-specific content in the private workspace. Apply the source-first gate: do not render or preview by default; use one auxiliary visual check only for an explicit request, an inherently visual deliverable or a concrete layout defect.
