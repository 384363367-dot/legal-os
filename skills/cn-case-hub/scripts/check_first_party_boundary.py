#!/usr/bin/env python3
"""Validate cn-case-hub first-party runtime dependency boundary."""
from pathlib import Path
import re,sys
ROOT=Path(__file__).resolve().parents[1]
legacy={'court_law_crawler.py','tax_law_crawler.py','moj_law_crawler.py','treaty_crawler.py','case_crawler.py'}
...
for p in [ROOT/'SKILL.md',*(ROOT/'references').glob('*.md')]:
    ...
    for line in p.read_text(encoding='utf-8',errors='ignore').splitlines():
        low=line.lower()
        negative=any(x in low for x in ('不依赖','无需','不得','不是','not require','does not require','must not require'))
...
print('FIRST_PARTY_BOUNDARY_PASS: cn-case-hub has no legacy crawler or third-party Skill/MCP SDK hard dependency')
