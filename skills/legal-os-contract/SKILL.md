...
- 修改 DOCX 时先做 OOXML、文本、字体、表格、批注和修订结构检查，再用 macOS Quick Look 原生预览；仅在中文字体、分页、表格、修订显示或打印版式存在疑点时使用 WPS/经批准的原生应用定向抽检。未经用户明确授权不得切换到其他渲染器。
...
8. 完成内容、金额、OOXML、原生预览、可访问性及隐私检查。只有公开脚本 `PASS`、退出码 `0`，风险相称的原生应用抽检完成，且组织要求的签核匹配当前文件哈希时，才可标 Final。
...
- 原文拒绝本次修订后的视图与源合同不一致；
- 接受修订后的视图与清洁版不一致；
- 长修订未精确批准、批注引用／范围锚点异常、原生预览或必要的 WPS/原生应用定向抽检未完成；
