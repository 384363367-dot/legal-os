...
- 修改 DOCX 时先做 OOXML、文本、字体、表格、批注和修订结构检查；仅在结构检查命中客观视觉风险（表格或合并单元格、分页或节、页眉页脚、图片、字体/编号、修订或批注显示）时，使用 macOS Quick Look 或经批准的原生 Office 应用定向抽检。没有触发项时不做全页视觉渲染；不得未经授权切换到其他渲染器。
...
8. 完成内容、金额、OOXML、可访问性及隐私检查；命中客观视觉风险时完成相称的原生应用定向抽检。只有公开脚本 `PASS`、退出码 `0`，基础结构检查通过，且组织要求的签核匹配当前文件哈希时，才可标 Final。
